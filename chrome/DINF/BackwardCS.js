...
function inferenceCS(inf){
        ...
        //
        this.inf.state = "inf"; // state = inferencing; the start state
        var triple;
        var uri;
        this.counter = 0;
       ...
       // handleParamTriples(inf)
        ...
        var time1 = myClock();
    //writeln("db: " + this.inf.dslist[0].ts);
    //writeln("Stack:::: "); printArray(this.inf.stack);
        testVerbose("GOALLIST: " + arrayToString( this.inf.goallist) + "\n");
    if (this.inf.triples == undefined || this.inf.triples == []){    // get all triples now
     ...
     this.inf.cache = [];
     if (! this.inf.nobuiltins){
         checkContradictionsB(this.inf);
     }
     this.inf.maxSteps = 0;
     this.inf.unSteps = 0;
     this.inf.nooutput = false;
     //this.inf.verbose = true;
     makePropertyDictI(this.inf);
     writeln("Backward.inference triples from propertyDictM: " + getAllTriplesM(this.inf));
     //writeln(propertyDictMToString(this.inf));
        // finite state loop

     this.performStep = function(){

         if(this.inf.state != "endinf"){

            this.counter++;
            if (this.inf.maxSteps > 0 && this.counter > this.inf.maxSteps){
                writeln(this.inf, "Stopped at maxSteps: " + this.inf.maxSteps);
                testVerbose(this.inf, "Stopped at maxSteps: " + this.inf.maxSteps);
                this.inf.state = "endinf";
            }
            // if the verbose flag is set give output
            testVerbose(inf,  " state ===== " + this.inf.state + "\n");
            // a solution has been found

            if (this.inf.state == "solution"){
                // show the solution
                testVerbose(this.inf, showsol( this.inf.sols[this.inf.sols.length-1], this.inf));
                // only one solution? Then stop.
                if (this.inf.onesol == 1)
                     this.inf.state = "endinf";
               else
                ...
                // print("matchlist after backtrack: " + this.inf.matchlist);
            ...
            // main state
            else if (this.inf.state == "inf"){
               var ret;
                //testVerbose(this.inf,"Stack: ");
                //testVerbose(this.inf,  arrayToString(this.inf.stack));
               if (this.inf.goallist.length != 0){
                  // check whether the next predicate is a builtin
                  // testVerbose(this.inf,"Substitutionlist: " + this.inf.subst);

                  var triple = this.inf.goallist[0];
                 ...
                 //print("triple1 Jsengine " + triple1);
                  ...
                  ret = 0;
                  if (! this.inf.nobuiltins){
                     ret = checkBuiltins(this.inf, triple1);
                ...
                if (ret == 0){
                      ...
                      testVerbose(this.inf, "Current Substitutions:\n"
                          + this.inf.subst + "\n");
                      //print("this.inf.sols    "+this.inf.sols + "\n");
                      solve(this.inf);
                      state = this.inf.state;
                }
            }
        }else{

             var time2 = myClock();
            // this.inf.infTime = time2 - time1;
            var sols = this.inf.sols;
            showsols(sols,this.inf);
            if(this.inf.abort){
	           ...
	           if (sols == []){
                  testNooutput(this.inf,  "There were no solutions found.\n");
               }else {
                  //testNooutput(this.inf, showsols(sols, this.inf));
               }
            }
        }

              testNooutput(this.inf, "Starting time BW: "  + time1 + "\n");
              testNooutput(this.inf, "Ending time BW: "  + time2 + "\n");
              testNooutput( this.inf, "Unifications steps: " + this.inf.unSteps);
              testNooutput(this.inf, "Unifications: " + this.inf.unifs);
//            print "Unifications/sec : ",inf.unifs/inf.infTime;
              testNooutput(this.inf, "Number of solutions: "
                              + this.inf.solnr + "\n");
   }
}
